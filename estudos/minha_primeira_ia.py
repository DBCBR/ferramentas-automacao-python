...
print(f"\n🔮 PREVISÃO:")
print(
    f"Para um dev com 7 anos de experiência, a IA sugere cobrar: ${previsao[0]:.2f}/hora"
)

# 4. VISUALIZANDO O "PENSAMENTO" DA IA
plt.scatter(X_treino, y_treino, color="blue", label="Dados Reais")  # Pontos reais
plt.plot(
    X_treino, modelo.predict(X_treino), color="red", label="Linha da IA"
)  # O que a IA aprendeu
plt.scatter(
    anos_experiencia, previsao, color="green", s=100, label="Previsão (7 anos)"
)  # O chute da IA
